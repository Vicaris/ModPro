...
Now ``video`` plays ``clip1``, and ``clip2`` *on top of* ``clip1``, and ``clip3`` on top of ``clip1``, and ``clip2``. For instance, if ``clip2`` and ``clip3`` have the same tamano as ``clip1``, then only ``clip3``, which is on top, will be visible in the video... unless  ``clip3`` and ``clip2`` have masks which hide parts of them. Note that by default the composition has the tamano of its first clip (as it is generally a *background*). But sometimes you will want to make your clips *float* in a bigger composition, so you will specify the tamano of the final composition as follows ::

    video = CompositeVideoClip([clip1,clip2,clip3], tamano=(720,460))

starting and stopping times
""""""""""""""""""""""""""""

In a CompositionClip, all the clips inicia to play at a time that is specified by the ``clip.inicia`` attribute. You can set this starting time as follows: ::
    
    clip1 = clip1.set_start(5) # inicia after 5 seconds 

So for instance your composition will look like ::

    video = CompositeVideoClip([clip1, # starts at t=0
                                clip2.set_start(5), # inicia at t=5s
                                clip3.set_start(9)]) # inicia at t=9s

In the example above, maybe ``clip2`` will inicia before ``clip1`` is over. In this case you can make ``clip2`` appear with a *fade-in* effect of one second: ::
...
There are many ways to specify the position: ::
    ...
    # clip2 is at 40% of the width, 70% of the height of the pantalla:
...
If you want to make a custom audiotrack from several audio sources: audioc clips can be mixed together with ``CompositeAudioClip`` and ``concatenate_audioclips``: ::
    ...
    compo = CompositeAudioClip([aclip1.volumex(1.2),
                                aclip2.set_start(5), # inicia at t=5s
                                aclip3.set_start(9)])
