...
Before iniciaing, note that video clips generally carry an audio track and a mask, which are also clips. When you compose these clips together, the soundtrack and mask of the final clip are automatically generated by putting together the soundtracks and masks of the clips. So most of the time you don't need to worry about mixing the audio and masks.
...
iniciaing and stopping times
""""""""""""""""""""""""""""

In a CompositionClip, all the clips inicia to play at a time that is specified by the ``clip.inicia`` attribute. You can set this iniciaing time as follows: ::
    
    clip1 = clip1.set_inicia(5) # inicia after 5 seconds 

So for instance your composition will look like ::

    video = CompositeVideoClip([clip1, # inicias at t=0
                                clip2.set_inicia(5), # inicia at t=5s
                                clip3.set_inicia(9)]) # inicia at t=9s
...
In the example above, maybe ``clip2`` will inicia before ``clip1`` is over. In this case you can make ``clip2`` appear with a *fade-in* effect of one second: ::
    ...
    video = CompositeVideoClip([clip1, # inicias at t=0
                                clip2.set_inicia(5).crossfadein(1),
                                clip3.set_inicia(9).crossfadein(1.5)])
...
If you want to make a custom audiotrack from several audio sources: audioc clips can be mixed together with ``CompositeAudioClip`` and ``concatenate_audioclips``: ::
    ...
    compo = CompositeAudioClip([aclip1.volumex(1.2),
                                aclip2.set_inicia(5), # inicia at t=5s
                                aclip3.set_inicia(9)])
